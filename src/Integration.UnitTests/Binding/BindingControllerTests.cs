...
namespace SonarLint.VisualStudio.Integration.UnitTests.Binding
{
    public class BindingControllerTests
    {
        ...
        [Fact]
        public async Task BindingController_BindCommand_Status()
        {
        ...
        [Fact]
        public async Task BindingController_BindCommand_Status_VsState()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
                ...
                // Act + Assert
                testSubject.BindCommand.CanExecute(projectVM)
                    .Should().BeFalse("No UI context: SolutionExistsAndFullyLoaded");
                ...
                // Act + Assert
                testSubject.BindCommand.CanExecute(projectVM)
                    .Should().BeFalse("No UI context: SolutionExistsAndNotBuildingAndNotDebugging");
                ...
                // Act + Assert
                testSubject.BindCommand.CanExecute(projectVM)
                    .Should().BeFalse("No managed projects");

                // Case 4: No projects at all
                solutionMock.RemoveProject(project1);
                // Act + Assert
                testSubject.BindCommand.CanExecute(projectVM)
                    .Should().BeFalse("No projects");
            });
        }

        [Fact]
        public async Task BindingController_BindCommand_Execution()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
        ...
        [Fact]
        public async Task BindingController_SetBindingInProgress()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
                ...
                foreach (var controllerResult in (ProgressControllerResult[])Enum.GetValues(typeof(ProgressControllerResult)))
                {
                    ...
                    // Assert
                    testSubject.BindCommand.CanExecute(projectVM)
                        .Should().BeFalse("Binding is in progress so should not be enabled");
                    ...
                    {
                        this.dteMock.ToolWindows.SolutionExplorer.Window.Active
                            .Should().BeFalse("SolutionExplorer window is not supposed to be activated");
                    }
                }
            });
        }
        ...
        [Fact]
        public async Task BindingController_BindingFinished()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
                ...
                foreach (ProgressControllerResult result in Enum.GetValues(typeof(ProgressControllerResult)).OfType<ProgressControllerResult>())
                {
                    // Arrange
                    testSubject.SetBindingInProgress(progressEvents, projectVM.ProjectInformation);
                    testSubject.IsBindingInProgress
                        .Should().BeTrue();
        ...
        [Fact]
        public async Task BindingController_BindingFinished_Navigation()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
        ...
        [Fact]
        public async Task BindingController_SetBindingInProgress_Notifications()
        {
            await TestHelper.StartSTATask(() =>
            {
                // Arrange
        ...
        private class TestBindingWorkflow : IBindingWorkflowExecutor
        {
            ...
            {
                this.BoundProject.Should().BeSameAs(expected, "Unexpected project binding");
            }
        }
