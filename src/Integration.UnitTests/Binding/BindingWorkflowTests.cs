    ...
    public class BindingWorkflowTests
    {
        ...
        public void BindingWorkflow_DownloadQualityProfile_Success()
        {
            ...
            var expectedOutput = string.Format(Strings.SubTextPaddingFormat,
                string.Format(Strings.QualityProfileDownloadSuccessfulMessageFormat, string.Empty, string.Empty, language.Name));
        ...
        private ConfigurablePackageInstaller PrepareInstallPackagesTest(BindingWorkflow testSubject, Dictionary<Language, IEnumerable<PackageName>> nugetPackages, params Project[] projects)
        {
            testSubject.BindingProjects.Clear();
            testSubject.BindingProjects.AddRange(projects);

            foreach (var kvp in nugetPackages)
            {
                testSubject.NuGetPackages.Add(kvp.Key, kvp.Value.Select(x => new NuGetPackageInfo { Id = x.Id, Version = x.Version.ToNormalizedString() }).ToList());
            }

            ConfigurablePackageInstaller packageInstaller = new ConfigurablePackageInstaller(nugetPackages.Values.SelectMany(x => x));
