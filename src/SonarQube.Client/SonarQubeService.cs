...
namespace SonarQube.Client
{
    public class SonarQubeService : ISonarQubeService, IDisposable
    {
        ...
        public SonarQubeService(string userAgent, ILogger logger)
            : this(new HttpClientHandlerFactory(new ProxyConfigurator(new SystemProxyProvider(), logger)), userAgent, logger, new RequestFactorySelector(), new SSEStreamReaderFactory(logger))
        {
        }
