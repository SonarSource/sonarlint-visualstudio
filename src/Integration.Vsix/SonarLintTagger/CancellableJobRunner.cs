...
namespace SonarLint.VisualStudio.Integration.Vsix.CancellableJob
{
    public interface ICancellableJobRunner
    {
        /// <summary>
        /// Runs a series of operations on a separate background thread.
        /// Can be cancelled.
        /// </summary>
        ICancellableJob Start(string jobDescription, IEnumerable<Action> operations, IProgress<JobRunnerProgress> progress, ILogger logger);
    }
    ...
    internal sealed class CancellableJobRunner : ICancellableJobRunner
    {
        public static ICancellableJobRunner Instance { get; } = new CancellableJobRunner();

        public ICancellableJob Start(string jobDescription, IEnumerable<Action> operations, IProgress<JobRunnerProgress> progress, ILogger logger)
        {
            var runner = new CancellableJob(jobDescription, operations, progress, logger);
        ...
        /// Can be cancelled.
        /// </summary>
        internal sealed class CancellableJob : ICancellableJob
        {
            ...
            internal /* for testing */ CancellableJob(string jobDescription, IEnumerable<Action> operations, IProgress<JobRunnerProgress> progress, ILogger logger)
            {
