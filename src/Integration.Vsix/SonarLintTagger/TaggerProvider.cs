...
internal sealed class TaggerProvider : ITaggerProvider, IRequireInitialization, IDocumentTracker, IDisposable
{
    ...
    public void AddIssueTracker(IIssueTracker issueTracker)
    {
        ...
        logger.LogVerbose(new MessageLevelContext(){VerboseContext = ["File Tracker Invoke", "Tagger"]}, "Open: {0}", filePath);
    ...
    {
        logger.LogVerbose(new MessageLevelContext(){VerboseContext = ["File Tracker Invoke", "Tagger"]}, "Save: {0}", fullPath);
