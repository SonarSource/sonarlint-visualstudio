...
namespace SonarLint.VisualStudio.Integration.Vsix.CFamily
{
    ...
    internal class CLangAnalyzer : IAnalyzer
    {
        ...
        private async Task TriggerAnalysisAsync(Request request, IIssueConsumer consumer, CancellationToken cancellationToken)
        {
            ...
            Action<Message> handleMessage = message => HandleMessage(message, request, consumer);
...
// duncanp TODO            logger.WriteLine($"Found {issues.Count} issue(s)");

            // Switch back to the UI thread
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
        }

        private void HandleMessage(Message message, Request request, IIssueConsumer consumer)
        {
