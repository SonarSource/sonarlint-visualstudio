...
namespace SonarLint.VisualStudio.Integration.Vsix.ErrorList
{
    ...
    /// </summary>
    internal sealed class EmptyIssuesSnapshot : IIssuesSnapshot
    {
        public static IIssuesSnapshot Create(string filePath)
            => new EmptyIssuesSnapshot(filePath);

        private EmptyIssuesSnapshot(string filePath)
        {
