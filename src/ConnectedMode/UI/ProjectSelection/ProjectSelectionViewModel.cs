    ...
    : ViewModelBase
{
    ...
    {
        var initializeProjectsParams = new TaskToPerformParams<ResponseStatus<List<ServerProject>>>(
    ...
    internal async Task<ResponseStatus<List<ServerProject>>> AdapterGetAllProjectsAsync()
    {
        ...
        if (!succeeded)
        {
            ...
            return new ResponseStatus<List<ServerProject>>(false, null);
        }
    ...
    internal void InitProjects(ResponseStatus<List<ServerProject>> response)
    {
    ...
    {
        var addManualProject = new TaskToPerformParams<ResponseStatus<ServerProject>>(
    ...
    private async Task<ResponseStatus<List<ServerProject>>> FuzzySearchProjectsAsync() =>
        await connectedModeServices.SlCoreConnectionAdapter.FuzzySearchProjectsAsync(ServerConnection, ProjectSearchTerm);

    private async Task<ResponseStatus<ServerProject>> FindServerProjectByKeyAsync(string projectKey) =>
