...
public class GetStatusResponseTests
{
    ...
    {
        var expected = new GetStatusResponse(isEnabled);
        var serialized = $$"""{"enabled":{{isEnabled.ToString().ToLower()}}}""";
        
        JsonConvert.DeserializeObject<GetStatusResponse>(serialized).Should().BeEquivalentTo(expected);
    }
}
