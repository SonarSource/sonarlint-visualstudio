...
[TestClass]
public class AddQuickFixAppliedForRuleParamsTests
{

    [TestMethod]
    public void Serialized_AsExpected()
    {
        var testSubject = new AddQuickFixAppliedForRuleParams("MyRule:Id");

        var serializeObject = JsonConvert.SerializeObject(testSubject);
        serializeObject.Should().Be("""{"ruleKey":"MyRule:Id"}""");
    }
}
