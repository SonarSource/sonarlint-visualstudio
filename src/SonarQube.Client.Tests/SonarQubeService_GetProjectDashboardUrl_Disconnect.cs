...
namespace SonarQube.Client.Tests
{
    ...
    public class SonarQubeService_GetProjectDashboardUrl_Disconnect : SonarQubeService_TestBase
    {
        ...
        {
            return new DisconnectingService(UserAgent, logger, requestFactorySelector);
        }

        internal class DisconnectingService : SonarQubeService
        {
            internal DisconnectingService(string userAgent, ILogger logger, IRequestFactorySelector requestFactorySelector)
                : base(userAgent, logger, requestFactorySelector, null)
            {
            }
