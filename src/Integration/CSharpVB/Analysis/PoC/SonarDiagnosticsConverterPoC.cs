...
public class SonarDiagnosticsConverterPoC() : ISonarDiagnosticsConverterPoC
{
    public IAnalysisIssue ConvertToAnalysisIssue(RoslynIssue diagnostic)
    {
        ...
        return new AnalysisIssue(
            ...
            primaryLocation,
            analysisIssueFlows,
            diagnostic.QuickFixes.Select(x => new QuickFix(x.Message,
                    x.Edits.Select(y => new Edit(y.NewText,
                            new TextRange(y.RangeToReplace.StartLine,
                                y.RangeToReplace.EndLine,
                                y.RangeToReplace.StartLineOffset,
                                y.RangeToReplace.EndLineOffset,
                                null)))
                        .ToList()))
                .ToList());
    }
