...
namespace SonarLint.VisualStudio.Integration.MefServices
{
    ...
    public sealed class MefSonarQubeService : SonarQubeService
    {
        ...
        protected override async Task<TResponse> InvokeUncheckedRequestAsync<TRequest, TResponse>(Action<TRequest> configure, HttpClient httpClientParam, CancellationToken token)
        {
            CodeMarkers.Instance.WebClientCallStart(typeof(TRequest).Name);

            Func<Task<TResponse>> asyncMethod = () => base.InvokeUncheckedRequestAsync<TRequest, TResponse>(configure, httpClientParam, token);
