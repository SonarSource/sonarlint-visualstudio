...
namespace SonarLint.VisualStudio.Roslyn.Suppressions.UnitTests
{
    ...
    public class SupportedSuppressionBuilderTests
    {
        ...
        {
            ReferenceEquals(SupportedSuppressionsBuilder.Instance, SupportedSuppressionsBuilder.Instance);
        }

        [TestMethod]
        public void SupportedSuppressionChecks()
        {
            ...
            result.Count().Should().Be(distinctIdCount);

            // Not null
            result.All(x => x != null).Should().BeTrue();

            // No utility analyzers
