...
public class UserSettingsProviderTests
{
    ...
    public void Ctor_NullArguments()
    {
        ...
        Action act = () => CreateUserSettingsProvider(null, new FileSystem(), mockSingleFileMonitor, mockSlCoreServiceProvider, mockRuleSettingsMapper);
        act.Should().ThrowExactly<ArgumentNullException>().And.ParamName.Should().Be("logger");

        act = () => CreateUserSettingsProvider(new TestLogger(), null, mockSingleFileMonitor, mockSlCoreServiceProvider, mockRuleSettingsMapper); 
        act.Should().ThrowExactly<ArgumentNullException>().And.ParamName.Should().Be("fileSystem");

        act = () => CreateUserSettingsProvider(new TestLogger(), new FileSystem(), null, mockSlCoreServiceProvider, mockRuleSettingsMapper);
    ...
    public void FileChanged_UpdateStandaloneRulesConfiguration_CallsRuleSettingsMapper()
    {
        ...
        var userSettingsProvider = SetupFileChangedInUserSettingsProvider(mockSlCoreServiceProvider, ruleSettingsMapper:rulesSettingsMapper);
    ...
    public void FileChanged_GettingSlCoreServiceProviderServiceFails_WritesALog()
    {
        ...
        mockLogger.Received(1).WriteLine(Arg.Is<string>(msg => msg.Contains(nameof(UserSettingsProvider)) && msg.Contains(SLCoreStrings.ServiceProviderNotInitialized)));
    }
    ...
    private static UserSettingsProvider CreateTestSubject(ILogger logger = null, IFileSystem fileSystem = null, 
        ISingleFileMonitor singleFileMonitor = null, ISLCoreServiceProvider slCoreService = null, IRuleSettingsMapper ruleSettingsMapper = null)
    {
        ...
        return CreateUserSettingsProvider(logger, fileSystem, singleFileMonitor, slCoreService, ruleSettingsMapper);
    }

    private static UserSettingsProvider CreateUserSettingsProvider(ILogger logger, IFileSystem fileSystem, 
        ISingleFileMonitor singleFileMonitor, ISLCoreServiceProvider slCoreService, IRuleSettingsMapper ruleSettingsMapper)
    {
        return new UserSettingsProvider(logger, fileSystem, singleFileMonitor, slCoreService, ruleSettingsMapper);
    }
    ...
    private static UserSettingsProvider SetupFileChangedInUserSettingsProvider(
        ISLCoreServiceProvider slCoreServiceProvider, 
        ILogger logger = null,
        IRuleSettingsMapper ruleSettingsMapper = null)
    {
        var fileMonitorMock = Substitute.For<ISingleFileMonitor>();
        var userSettingsProvider = CreateUserSettingsProvider(
            logger ?? Substitute.For<ILogger>(),
