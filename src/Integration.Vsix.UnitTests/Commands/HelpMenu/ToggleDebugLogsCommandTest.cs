...
{
    [TestClass]
    public class ToggleDebugLogsCommandTests
    {
        [TestMethod]
        public void Invoke_VerifyTogglingWorks()
        {
            var command = CommandHelper.CreateRandomOleMenuCommand();

            var testSubject = new ToggleDebugLogsCommand();

            testSubject.Invoke(command, null);
            testSubject.QueryStatus(command, null);
            command.Text.Should().Be("Disable Debug Logs");

            testSubject.Invoke(command, null);
            testSubject.QueryStatus(command, null);
            command.Text.Should().Be("Enable Debug Logs");
        }
    }
}
