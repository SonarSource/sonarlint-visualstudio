...
namespace SonarLint.VisualStudio.Integration.UnitTests.CFamily
{
    ...
    public class DynamicRulesConfigurationTests
    {
        ...
        public void ActiveRulesMergedCorrectly()
        {
            ...
            var standaloneConfig = new UserSettings
            {
                Rules = new Dictionary<string, RuleConfig>
                {
                    // Unknown rules should be ignored
                    { "unknown1", new RuleConfig { Level = RuleLevel.On } },
                    { "unknown2", new RuleConfig { Level = RuleLevel.Off } },

                    // Turn on a rule that was off (case-insensitive comparison on keys)
                    { "rule1", new RuleConfig { Level = RuleLevel.On } },

                    // Turn off a rule that was on
                    { "rule2", new RuleConfig { Level = RuleLevel.Off} },

                    // Rule key comparison is case-sensitive
                    { "RULE3", new RuleConfig { Level = RuleLevel.Off} }

                }
            };

            // Act
            var dynamicConfig = DynamicRulesConfiguration.CalculateActiveRules(defaultConfig, standaloneConfig);
        ...
        public void Ctor_NoSettings_DefaultsUsed()
        {
            ...
            // Assert
            dynamicConfig.ActiveRuleKeys.Should().BeEquivalentTo("rule1");
        }
        ...
        public void Ctor_ErrorLoadingSettings_ErrorSquashedAndDefaultsUsed()
        {
            ...
            // Assert
            dynamicConfig.ActiveRuleKeys.Should().BeEquivalentTo("ruleX");
...
";
            ...
            // Assert
            dynamicConfig.ActiveRuleKeys.Should().BeEquivalentTo("rule1", "rule4");
        }
    }
}
