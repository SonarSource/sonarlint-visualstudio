...
public class SLCoreEmbeddedPluginJarProviderTests
{
    ...
    private void MockFileSystem() => fileSystem.Directory.Returns(directory);

    private void MockVsixLocator() => vsixRootLocator.GetVsixRoot().Returns("C:\\VsixRoot");

    private static string BuildJarFullPath(string jarFileName) => Path.Combine("C:\\VsixRoot", jarFileName);
