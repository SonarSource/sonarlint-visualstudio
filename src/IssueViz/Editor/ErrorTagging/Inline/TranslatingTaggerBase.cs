...
{
    /// <summary>
    /// Base class for a tagger that produces tags that are derived from another tag type
    /// e.g. a tagger that produces IErrorTags based on our IIssueLocationTags.
    ///
    /// NB This class differs from the FilteringTaggerBase in that it does not assume there is a one-to-one
    /// mapping between the tracked tags and the tags that are produced
    /// e.g. there could be three tracked tags on a single line that are translated to a single "target" tag
    /// </summary>
    ...
    internal abstract class TranslatingTaggerBase<TTrackedTagType, TTagType> : ITagger<TTagType>, IDisposable
        ...
        where TTagType : ITag
    {
        ...
        protected TranslatingTaggerBase(ITagAggregator<TTrackedTagType> tagAggregator, ITextBuffer textBuffer)
            : this(tagAggregator)
        {
            this.buffer = textBuffer;
        }

        protected TranslatingTaggerBase(ITagAggregator<TTrackedTagType> tagAggregator, ITextView textView)
            : this(tagAggregator)
        {
            this.view = textView;
        }

        private TranslatingTaggerBase(ITagAggregator<TTrackedTagType> tagAggregator)
        {
        ...
        protected abstract TagSpan<TTagType> CreateTagSpan(TTagType targetTag, NormalizedSnapshotSpanCollection spans);

        /// <summary>
        /// Translates the set of tracked tag spans to a set of target spans
        /// </summary>
        protected abstract IEnumerable<IMappingTagSpan<TTagType>> Translate(IEnumerable<IMappingTagSpan<TTrackedTagType>> trackedTagSpans);
        ...
        public IEnumerable<ITagSpan<TTagType>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            var translatedTagSpans = Translate(trackedTagSpans);

            var callerSnapshot = spans[0].Snapshot;

            foreach (var dataTagSpan in translatedTagSpans)
            {
