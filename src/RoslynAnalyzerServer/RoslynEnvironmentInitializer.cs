...
public interface IRoslynEnvironmentInitializer : IRequireInitialization, IDisposable;
...
internal class RoslynEnvironmentInitializer(
    ...
    : IRoslynEnvironmentInitializer
{
    public IInitializationProcessor InitializationProcessor { get; } = initializationProcessorFactory.CreateAndStart<RoslynEnvironmentInitializer>(
        [analyzerAssemblyContentsLoader, suppressionExclusionConfigGenerator],
        () =>
        {
            _ = roslynAnalysisHttpServer.StartListenAsync().ConfigureAwait(false); // File and forget
        });
