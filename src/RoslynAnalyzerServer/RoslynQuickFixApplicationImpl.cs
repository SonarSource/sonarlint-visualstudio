...
public class RoslynQuickFixApplicationImpl
{
    ...
    public async Task ApplyAsync(CancellationToken cancellationToken)
    {
        ...
        {
            if (codeActionOperation is not ApplyChangesOperation applyChangesOperation)
            {
                // todo not applychangesoperation?
                continue; // todo??
            }

            await workspace.ApplyOrMergeChangesAsync(originalSolution, applyChangesOperation, cancellationToken);
        }
    }
}
